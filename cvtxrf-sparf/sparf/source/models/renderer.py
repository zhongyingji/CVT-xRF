...
from source.utils import camera, ray_insertion
from source.utils.torch import to_cuda
...
class Graph(torch.nn.Module):
    ...
    def define_renderer(self, opt: Dict[str, Any]):
        ...
        if opt.use_transformer: 
            self.transformer = Transformer(opt).to(self.device)
    ...
    def render_image_at_specific_rays(self, opt: Dict[str, Any], data_dict: Dict[str, Any], iter: int, 
        ...
        if dataloader_rays is not None: 
            # during training
            ret_from_loader = self.render_dataloader_rays(opt, dataloader_rays, depth_range=data_dict.depth_range[0], iter=iter, mode=mode)
            if ray_idx is None:
                # only rays from dataloader
                raise NotImplementedError
            
            ret_dict_loader = dict(
                spc_loader=ret_from_loader)
    ...
    def render(self, opt: Dict[str, Any], pose: torch.Tensor, H: int, W: int, intr: torch.Tensor, pixels: torch.Tensor = None, 
        ...
        pred_coarse = self.nerf.forward_samples(opt,center,ray,depth_samples,
                                                ...
                                                # {rgb_samples: [B, n_rays, n_samples, 3], density_samples: [B, n_rays, n_samples, 1]}
        # print("forward samples coarse done. ")
        ...
        if opt.nerf.fine_sampling and compute_fine_sampling:
            ...
            pred_fine = self.nerf_fine.forward_samples(opt,center,ray,depth_samples,
                                                       embedder_pts=self.embedder_pts, 
                                                       embedder_view=self.embedder_view, mode=mode)
            # print("forward samples fine done. ")
    ...
    def render_by_slices(self, opt: Dict[str, Any], pose: torch.Tensor, H: int, W: int, 
        ...
        if opt.nerf.fine_sampling:
            if (hasattr(opt.nerf, 'ratio_start_fine_sampling_at_x') and opt.nerf.ratio_start_fine_sampling_at_x is not None \
                ...
                ret_all.update(rgb_fine=[],rgb_var_fine=[], depth_fine=[],
                            ...
                            rgb_samples_fine=[], density_samples_fine=[], points_3d_fine=[])
        ...
        for c in range(0,H*W,opt.nerf.rand_rays):
            ...
            """
            for k in ret.keys(): 
                if k in ret_all.keys(): ret_all[k].append(ret[k])
                else: ret_all[k] = [ret[k]]
            """
    ...
    def render_dataloader_rays(self, opt: Dict[str, Any], dataloader_rays: Dict[str, Any], 
                            depth_range: List[float] = None, iter: int = None, mode: int = None) -> Dict[str, Any]:
        """Main rendering function for rays from dataloader
        ...
        if pixels is not None:
            ...
            # from ray_idx
            center,ray = camera.get_center_and_ray(pose, H, W, intr=intr) # [B,HW,3]
            print("in render function, center.shape: {}.".format(center.shape))
            while ray.isnan().any(): # TODO: weird bug, ray becomes NaN arbitrarily if batch_size>1, not deterministic reproducible
                center,ray = camera.get_center_and_ray(pose, H, W,intr=intr) # [B,HW,3]
        ...
        # to gpu
        dataloader_rays = to_cuda(dataloader_rays, self.device)
        ray_idx, center, ray, target_s, ref_pts, t_ref_pts, sur_pts, t_nxt_ray_pts, vox_end = dataloader_rays
        # ray_idx: [N_rays, 1]
        # center: [N_rays, 3]
        # ref_pts: [N_rays, 3]
        # t_ref_pts: [N_rays, 1]
        # sur_pts: [N_rays, 3, n_sur_pts+n_nxt_ray_pts]
        # t_nxt_ray_pts: [N_rays, n_nxt_ray_pts]
        # vox_end: [N_rays, 1]
        # print("Check shape in render_dataloader_rays(...). ray_idx: {}, center: {}.".format(ray_idx.shape, center.shape))

        
        tmp_pts = torch.cat([ref_pts[:, None, :], torch.transpose(sur_pts, 1, 2)], dim=1) # [N_rays, 1+n_aug_pts, 3]

        pred = edict(origins=center, viewdirs=ray, dataloader_ray_targets=target_s)

        N_rays = center.shape[0]
        ...
        depth_samples = self.sample_depth(opt,batch_size,num_rays=ray.shape[1], 
                                            ...
                                            depth_range=depth_range, mode=mode) # [B,HW,N,1] or [B, N_rays, N, 1]
        """
        # overwrite the self.sample_depth()
        rand_samples = torch.rand(1, N_rays, opt.nerf.sample_intvs, 1, device=self.device) # [1, N_rays, N, 1]
        rand_samples += torch.arange(opt.nerf.sample_intvs,device=self.device)[None, None, :, None].float() # [1, N_rays, N, 1]
        depth_samples = rand_samples / opt.nerf.sample_intvs*(depth_max-depth_min) + depth_min # [1, N_rays, N, 1]

        pred_coarse = self.nerf.forward_samples(opt, center[None], ray[None], depth_samples,
                                                embedder_pts=self.embedder_pts, 
                                                embedder_view=self.embedder_view, mode=mode)
        # {rgb_samples: [B, n_rays, n_samples, 3], density_samples: [B, n_rays, n_samples], 
        # density_feature: [B, n_rays, n_samples, 256], rgb_feature: None}
        pred_coarse['t'] = depth_samples
        # print("forward_samples coarse transformer done. ")

        
        pred_coarse = self.nerf.composite(opt,ray[None],pred_coarse,depth_samples)
        # new elements are rgb,depth,depth_var, opacity,weights,loss_weight_sparsity 
        # [B,HW,K] or [B, N_rays, K], loss_weight_sparsity is [B]
        weights = pred_coarse['weights']
        ...
        if opt.nerf.fine_sampling and compute_fine_sampling:
            ...
            depth_samples = torch.cat([depth_samples,depth_samples_fine],dim=2) # [B,HW,N+Nf,1]
            depth_samples = depth_samples.sort(dim=2).values
            z_vals = depth_samples[0, ..., 0] # [N_rays, N+Nf]
            
            # NOTE: get the coordinate of the points here
            points_3D_samples = camera.get_3D_points_from_depth(center[None], ray[None], depth_samples, 
                                                                multi_samples=True) # [B,HW,N+Nf,3], HW=N_rays
            aug_points_3D_samples = torch.cat([points_3D_samples, tmp_pts[None]], dim=2) # [B, N_rays, N+Nf+1+n_aug_pts, 3]
            
            pred_fine = self.nerf_fine.forward_samples(opt, center[None], ray[None], depth_samples=None, # for safe check
                                                       embedder_pts=self.embedder_pts, 
                                                       embedder_view=self.embedder_view, mode=mode, points_3D_samples=aug_points_3D_samples)
            # print("forward_samples fine transformer done. ")
            """
            pred_fine = dict(
                rgb_samples=rgb,  # [B, n_rays, N+Nf+1+n_aug_pts, 3]
                density_samples=density, # [B, n_rays, N+Nf+1+n_aug_pts]
                density_feature=feat_density, # [B, n_rays, N+Nf+1+n_aug_pts, 256]
                rgb_feature=None, 
            )
            # NOTE: density samples do not have last dim
            """
            raw = torch.cat([
                pred_fine["rgb_samples"][0, :, :(opt.nerf.sample_intvs+opt.nerf.sample_intvs_fine)], 
                pred_fine["density_samples"][0, :, :(opt.nerf.sample_intvs+opt.nerf.sample_intvs_fine), None]], dim=-1) # [N_rays, N+Nf, 4]
            
            ray_norm = ray / torch.norm(ray, dim=-1, keepdim=True)
            ray_norm_expand = ray_norm[:, None].expand(tmp_pts.shape)

            # NOTE: apply transformer on the prediction, only support the batchsize=1
            alph_embed_sur = pred_fine["density_feature"][0, :, (opt.nerf.sample_intvs+opt.nerf.sample_intvs_fine):, :] # [N_rays, 1+n_aug_pts, 256]
            rgb_embed_sur = pred_fine["rgb_feature"][0, :, (opt.nerf.sample_intvs+opt.nerf.sample_intvs_fine):, :] # [N_rays, 1+n_aug_pts, 128]
            pred_transformer = self.transformer(
                opt, sur_pts=tmp_pts, alph=pred_fine["density_samples"][0, ..., None], alph_embed=alph_embed_sur, 
                rgb=pred_fine["rgb_samples"][0], rgb_embed=rgb_embed_sur, viewdirs=ray_norm_expand, 
                embedder_pts=self.embedder_pts, embedder_view=self.embedder_view, mode=mode
            )

            insert_pts = {
                "rgb_ref": pred_transformer["rgb"][:, [0]], 
                "rgb_nxt_ray": pred_transformer["rgb"][:, -opt.voxel_sampler.n_nxt_ray_pts:], 
                "alph_ref": pred_transformer["alph"][:, [0]], 
                "alph_nxt_ray": pred_transformer["alph"][:, -opt.voxel_sampler.n_nxt_ray_pts:], 
                "ts_ref": t_ref_pts, 
                "ts_nxt_ray": t_nxt_ray_pts, 
                "vox_end": vox_end, 
            }

            insertion_results = ray_insertion.insert_pt_to_ray(raw, z_vals, insert_pts)

            # overwrite the prediction
            pred_fine = dict(
                rgb_samples=insertion_results["inserted_ray_rgb_alph"][None, ..., :3], # [B, N_rays, N+Nf+1+n_nxt_ray_pts, 3]
                density_samples=insertion_results["inserted_ray_rgb_alph"][None, ..., -1], # [B, N_rays, N+Nf+1+n_nxt_ray_pts] 
                surround_embed=pred_transformer["surround_embed"], # [N_rays, embed_dim]
                transformer_output_alpha=insertion_results["transformer_output_alpha"], 
            )
            pred_fine['t'] = insertion_results["inserted_zvals"][None, ..., None] # [B, N_rays, N+Nf+1+n_nxt_ray_pts, 1]
            # pred_fine['t'] = depth_samples
            pred_fine = self.nerf_fine.composite(opt, ray[None], 
                                                pred_fine, pred_fine['t'])
    ...
    def sample_depth(self, opt: Dict[str, Any], batch_size: int, n_samples: int, H: int, W: int, depth_range: List[float], \
        ...
        if opt.nerf.sample_stratified and mode not in ['val', 'eval', 'test']:
            ...
            rand_samples = 0.5 * torch.ones(batch_size,num_rays,n_samples,1,device=self.device) # [B, 341, 128, 1]
