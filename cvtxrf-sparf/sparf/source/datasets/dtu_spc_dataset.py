...
from source.datasets.utils.ray_voxel_intersection import ray_voxel_intersect
from source.datasets.spc_utils import GridPointsSampler, VoxelStorageDS


class DTUSPCDataset(Dataset): 
    def __init__(self, opt, all_data, 
                pre_save_dir="./spc_cache/", chunk=64*1024, device=None): 
        super().__init__()
        print("DTUSPCDataset with self.train_data.all as initialization. ")

        self.opt = opt
        self.all_data = all_data

        """
            self.all_data, e.g., 9-view, 
            idx torch.Size([9])
            rgb_path
            depth_gt torch.Size([9, 300, 400])
            fg_mask torch.Size([9, 1, 300, 400])
            valid_depth_gt torch.Size([9, 300, 400])
            image torch.Size([9, 3, 300, 400])
            intr torch.Size([9, 3, 3])
            pose torch.Size([9, 3, 4])
            depth_range torch.Size([9, 2])
            scene
        """

        self.n_train_imgs, _, H, W = self.all_data["image"].shape
        self.scene = "DTU_{}".format(opt.scene)

        center_all, ray_all = self.get_all_rays(H, W)
        # [B, HW, 3]
        assert center_all.dim() == 3

        self.center_all, self.ray_all = center_all.numpy(), ray_all.numpy()
        self.center_all = self.center_all.reshape(-1, 3) # [B*HW, 3]
        self.ray_all = self.ray_all.reshape(-1, 3) # [B*HW, 3]
        self.imgs = self.all_data["image"].permute(0, 2, 3, 1).numpy() # [B, H, W, 3]

        self.near, self.far = self.all_data["depth_range"][0].numpy()
        self.scene_range = self.get_scene_range(self.near, self.far)
        self.device = device

        self.voxel_num = opt.voxel_sampler.voxel_num
        self.voxel_size = self.scene_range / opt.voxel_sampler.voxel_num
        ...
        self.save_dir = pre_save_dir+"{}_voxel_flag_{}x{}_{}vox_range{}_{}images_sparfreader".format(
            self.scene, H, W, self.voxel_num, str(self.scene_range), self.n_train_imgs)
        self.prefix = "{}x{}_{}vox_range{}_{}images_sparfreader".format(
            H, W, self.voxel_num, str(self.scene_range), self.n_train_imgs)
        print("Scene: {}, Save_dir: {}, prefix: {}".format(self.scene, self.save_dir, self.prefix))

        self.voxel_flag, self.in_voxel_num = None, None
        if not os.path.exists(self.save_dir):
            print("Make the new directory {} and writing...".format(self.save_dir))
            ...
            print("Write done. Kill the process.")
            sys.exit()
        else: 
            print("Directory {} exists. Directly read from it. ".format(self.save_dir))
            self.in_voxel_sum = np.load(os.path.join(self.save_dir, self.prefix+"_voxel_sum.npy"))


        self.n_sur_pts = self.opt.voxel_sampler.n_sur_pts
        self.n_nxt_ray_pts = self.opt.voxel_sampler.n_nxt_ray_pts
        self.grid_sampler = GridPointsSampler(self.n_sur_pts, self.voxel_size, "split_region")
    
    def get_all_rays(self, H, W): 
        center_all, ray_all = camera.get_center_and_ray(
            pose_w2c=self.all_data["pose"], H=H, W=W, intr=self.all_data["intr"])
        # [B, HW, 3]
        return center_all, ray_all
    
    def get_scene_range(self, near, far): 
        rays_o = self.center_all
        rays_d = self.ray_all
    ...
    def get_ray_voxel_intersect(self, chunk):
        ...
        rays_o = torch.Tensor(self.center_all).to(self.device)
        rays_d = torch.Tensor(self.ray_all).to(self.device)
        ...
        for i in range(0, rays_o.shape[0], chunk):
            ...
            """
            tmp_rays_o = rays_o[i:i+n_rays]
            tmp_rays_d = rays_d[i:i+n_rays]
            print("Important checking...")
            print(tmp_rays_o[0] + ts[0, 0] * tmp_rays_d[0], tmp_rays_o[0]+ts[0, 1]*tmp_rays_d[0])
            print("Check if it is identical to: {}, {}".format(
                coord[0, 0]*self.voxel_size+self.xyzmin, 
                coord[0, 1]*self.voxel_size+self.xyzmin))
            print("Check if the ts are reasonable: {}".format(ts[0]))
            print("Check done.")
            """
            ...
            target_rgb = torch.Tensor(self.imgs.reshape(-1, 3)).to(self.device) # (N*HW, 3)
            target_rgb = target_rgb[rays_idx.view(-1).long()]
            # voxel_info = torch.cat([rays_idx, coord_in, coord_out, ts_flat], -1) # (all_intersected_voxels, 5)
    ...
    def data_from_record(self, voxel_idx, record, n_sample_pts):
        ...
        ray_o = self.center_all[ray_idx]
        ray_d = self.ray_all[ray_idx]

        ray_rgb = self.imgs.reshape(-1, 3)[ray_idx]
        ...
        return ray_idx, ray_o, ray_d, ray_rgb, ref_pt, t_ref_pt, sur_pts
    ...
    def __getitem__(self, idx):
        ...
        ray_idx, ray_o, ray_d, ray_rgb, ref_pt, t_ref_pt, sur_pts = self.data_from_record(voxel_idx, record, self.n_sur_pts)
        ...
        return ray_idx, ray_o, ray_d, ray_rgb, ref_pt, t_ref_pt, \
...
def single_proc_write(save_dir, prefix, voxel_flag, voxel_sum):
    ...
    for ridx in range(len(voxel_flag)):
        ...
        np.save(save_file_path, record)
        print("ridx: {}. save_file_path: {}.".format(ridx, save_file_path))
        # if ridx == 10000: 
        #     return
...
if __name__ == "__main__": 
    # from source.datasets.dtu import DTUDatasetPixelNerf
    from source.datasets.create_dataset import create_dataset
    
    voxel_num = 64
    
    # TODO: read the data from DTU
