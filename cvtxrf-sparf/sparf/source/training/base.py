...
class PerSceneTrainer(IterBasedTrainer):
    ...
    def run(self, load_latest: bool=True, make_validation_first: bool=False):
        ...
        self.save_rendered_val_during_inference = False
        ...
        while self.iteration < self.max_iteration:
            ...
            if self.iteration % self.settings.val_steps == 0:
                ...
                # render test during validation
                self.save_rendered_val_during_inference = True
                self.save_rendered_val_dir = os.path.join('{}/{}'.format(self._base_save_dir, self.settings.project_path), 
                                                'rendertest_iter{}/'.format(self.iteration))
                if not os.path.exists(self.save_rendered_val_dir):
                    os.makedirs(self.save_rendered_val_dir, exist_ok=True)
                ...
                if self.current_best_val is not None and self.current_best_val < self.best_val:
                    ...
                    # TODO: save the images message
                    message_rendertest = 'Rendered val images are saved at {}'.format(self.save_rendered_val_dir)
                    self.logger.critical(message_rendertest)
                
                # for safety consideration, not sure whether other functions will call val_step(...)
                self.save_rendered_val_during_inference = False
    ...
    def val_step(self, iteration: int, data_dict: Dict[str, Any]
        ...
        # print("Check shape in val_step. gt_image: {}. pred_rgb_map: {}. idx: {}. idx_: {}".format(
        #     gt_image.shape, pred_rgb_map.shape, data_dict['idx'], os.path.basename(data_dict['rgb_path'][0].split('.')[0])))
        # Check shape in val_step. gt_image: torch.Size([1, 3, 300, 400]). pred_rgb_map: torch.Size([1, 3, 300, 400]). 
        # idx: tensor([0], device='cuda:0'). idx_: /ssddata/yzhongbn/research_project/sparf/dtu/rs_dtu_4/DTU/scan82/image/000001
        ...
        if 'rgb_fine' in output_dict.keys():
            pred_rgb_map_fine = output_dict.rgb_fine.reshape(-1, H, W, 3).permute(0,3,1,2)
            ssim = ssim_loss(pred_rgb_map_fine, gt_image).item()
            lpips = self.lpips_loss(pred_rgb_map_fine*2-1, gt_image*2-1).item()

            results_dict['ssim_fine'] = ssim
            results_dict['lpips_fine'] = lpips
            
            if 'fg_mask' in data_dict.keys():
                results_dict.update(compute_metrics_masked(data_dict, pred_rgb_map_fine, gt_image, 
                                                           self.lpips_loss, suffix='_fine'))
        
        # ------------------------------------------------------------------------
        # the following code is borrowed from function def visualize_eval(...)
        if self.save_rendered_val_during_inference: 
            file_id = os.path.basename(data_dict['rgb_path'][0].split('.')[0])
            gt_image = (gt_image.permute(0, 2, 3, 1)[0].cpu().numpy() * 255.).astype(np.uint8) # (H, W, 3)
            pred_rgb_map = pred_rgb_map.permute(0, 2, 3, 1)[0].cpu().numpy() # (H, W, 3)
            pred_rgb_map_np_uint8 = (255 * np.clip(pred_rgb_map, a_min=0, a_max=1.)).astype(np.uint8)
            pred_rgb_map_fine = pred_rgb_map_fine.permute(0, 2, 3, 1)[0].cpu().numpy() # (H, W, 3)
            pred_rgb_map_fine_np_uint8 = (255 * np.clip(pred_rgb_map_fine, a_min=0, a_max=1.)).astype(np.uint8)
            
            name = '{}_rgb_coarse.png'.format(file_id)
            name_fine = '{}_rgb.png'.format(file_id)
            name_gt = '{}_gt.png'.format(file_id)
            imageio.imwrite(os.path.join(self.save_rendered_val_dir, name), pred_rgb_map_np_uint8)
            imageio.imwrite(os.path.join(self.save_rendered_val_dir, name_fine), pred_rgb_map_fine_np_uint8)
            imageio.imwrite(os.path.join(self.save_rendered_val_dir, name_gt), gt_image)
