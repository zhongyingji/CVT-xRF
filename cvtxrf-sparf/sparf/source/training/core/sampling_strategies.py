...
class RaySamplingStrategy:
    ...
    def __init__(self, opt: Dict[str, Any], data_dict: Dict[str, Any], device: torch.device):
        ...
        """
        self.replace_sparf_sampler = opt.voxel_sampler.replace_sparf_sampler
        self.voxel_based_ray_dataloader = None
        print("[RaySamplingStrategy]: replace_sparf_sample: {}. ".format(self.replace_sparf_sampler))
        if dataloader in data_dict: 
            voxel_based_ray_dataloader = data_dict.dataloader
            self.voxel_based_ray_dataloader = iter(voxel_based_ray_dataloader)
        """
    ...
    def __call__(self, nbr_pixels: int, sample_in_center: bool=False, 
                 idx_imgs: List[int]=None) -> Dict[str, Any]: 

        """
        sampled_rays = {"random_rays": None}
        
        if self.voxel_based_ray_dataloader is not None:
            # sampled_rays["dataloader_rays"] = {}
            
            data_from_loader = next(self.voxel_based_ray_dataloader)
            # center, ray, target_s, ref_pts, t_ref_pts, sur_pts, t_nxt_ray_pts, vox_end = data_from_loader
            sampled_rays["dataloader_rays"] = data_from_loader
            if self.replace_sparf_sampler: 
                # sample the rays only from dataloader
                return sampled_rays
                
            # concat the rays from dataloader and original sampler
        """ 
        ...
        # print("nbr_pixels: {}, nbr_images: {}, nbr_rand_pixels_per_img: {}".format(nbr_pixels, nbr_images, nbr_rand_pixels_per_img))
        ...
        # print("in sampling strategy, random_rays.shape: {}. random_pixels.shape: {}. ".format(random_rays.shape, random_pixels.shape))
        # in sampling strategy, random_rays.shape: torch.Size([341]). random_pixels.shape: torch.Size([341, 2]).
        return random_rays # indices
